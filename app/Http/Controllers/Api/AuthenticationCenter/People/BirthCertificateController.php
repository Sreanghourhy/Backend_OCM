...
class BirthCertificateController extends Controller
{
    ...
    public function create(Request $request){
        ...
        $record = RecordModel::create([
            ...
            'year' => strlen( $request->year ) > 0 ? \Carbon\Carbon::parse( $request->year )->format('Y-m-d') : \Carbon\Carbon::now()->format('Y') ,
            ...
            'dob' => strlen( $request->dob ) > 0 ? \Carbon\Carbon::parse( $request->dob )->format('Y-m-d') : \Carbon\Carbon::now()->format('Y-m-d') ,
            'gender' => $request->gender?? 'male' ,
            'nationality' => $request->nationality?? 'ខ្មែរ' ,
            'national' => $request->national?? 'ខ្មែរ' ,
            'pob' => $request->pob?? '' ,
            'issued_date' => strlen( $request->issued_date ) > 0 ? \Carbon\Carbon::parse( $request->issued_date )->format('Y-m-d') : \Carbon\Carbon::now()->format('Y-m-d') ,
    ...
    public function update(Request $request){
        if( isset( $request->id ) && $request->id > 0 && ( $record = RecordModel::find($request->id) ) !== null ){
            ...
            if( $record->update([
                'birth_number' => $request->birth_number?? '' ,
                'book_number' => $request->book_number?? '' ,
                'year' => strlen( $request->year ) > 0 ? \Carbon\Carbon::parse( $request->year )->format('Y-m-d') : \Carbon\Carbon::now()->format('Y-m-d') ,
                'province_id' => $request->province_id?? 0 ,
                'district_id' => $request->district_id?? 0 ,
                'commune_id' => $request->commune_id?? 0 ,
                'profession' => $request->profession??'' ,
