...
use App\Models\Meeting\Organization;
...
class OfficerUnofficialController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $builder->where('additional_officer_type', 'admin_unofficial');
    ...
    public function getOfficerUnofficialByIds(Request $request){
     ...
     * Create an account
     */
    public function storeUnofficialOfficer(Request $request){
        ...
        $officer = $people->officers()->create([
            ...
            'additional_officer_type' => $request->additional_officer_type ?? 178,
        ...
        if( $user ){
            ...
            if( isset( $_FILES['files']['tmp_name'] ) && $_FILES['files']['tmp_name'] != "" ) {
                if( ( $officer = RecordModel::find($request->id) ) !== null ){
                    ...
                    if( $officer->image != null &&Storage::disk('public')->exists( $officer->image ) ){
                        Storage::disk('public')->delete( $officer->image );
                    }
                    $uniqeName = Storage::disk('public')->putFile( 'officer_unofficial_image/'.$officer->id , new File( $_FILES['files']['tmp_name'] ) );
